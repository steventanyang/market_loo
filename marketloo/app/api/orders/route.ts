...
function calculatePriceWithImpact(currentPrice: number, amount: number, type: 'buying' | 'selling') {
  // Base spread and liquidity variation
  const baseVariation = BASE_SPREAD + (Math.random() * LIQUIDITY_FACTOR * 2 - LIQUIDITY_FACTOR);
...
export async function POST(request: Request) {
  try {
    ...
    const { market_id, user_id, outcome_id, amount, type } = body;

    // Validate input
    if (!market_id || !user_id || !outcome_id || !amount || !type) {
      ...
      for (const matchingOrder of matchingOrders) {
        ...
        const { error: updateMatchError } = await supabase
          ...
          .update({
            ...
            status: matchingOrder.amount - tradeAmount === 0 ? 'filled' : 'pending'
          })
        ...
        await updatePricesWithConstraint(outcome_id, market_id, matchingOrder.price);
    ...
    if (remainingAmount > 0) {
      console.log('Creating market maker order for remaining amount:', remainingAmount);
      ...
      console.log('Market Maker Price Update:', {
        ...
        change: ((priceVariation - outcome.current_price) / outcome.current_price) * 100 + '%'
      });
      ...
      const { error: mmTradeError } = await supabase
        .from('trades')
        .insert({
          market_id,
